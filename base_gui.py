...
class MainWindow(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        self.inspectBtn.clicked.connect(self._on_inspect)
    ...
    def _on_inspect(self):
        ...
        if ID is None:
            return
        out = self._call_captured(core.inspect, ID)
        self.detailView.setHtml(ansi_to_html(out) or "No output.")

    def _on_list_tags(self):
        ID = self._selected_id()
        if ID is None:
            return
        out = self._call_captured(core.list_tags, ID)
        self.detailView.setHtml(ansi_to_html(out) or "No tags.")
    ...
    def _on_display(self):
        ...
        out = self._call_captured(core.display, type_)
        self.detailView.setHtml(ansi_to_html(out) or "No matching entries.")
    ...
    def _on_manage_load(self):
        ...
        if not ID:
            ...
            return
        out = self._call_captured(core.info, self._to_info_query(ID))
        self.manageInfoView.setHtml(ansi_to_html(out) or "No matching entry.")
